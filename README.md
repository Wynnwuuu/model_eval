...
当前内置第四轮 **原 30 条音频 × 2 个模型 × 2 版 Prompt**：Doubao Seed 2.0 Lite(260428) 与 Qwen3.5-Omni-Plus，分别使用结构化和 Wynn结构化 Prompt（Schema 2.2），每条对应四份分析，完整矩阵共 120 份结果。普通评审无需 API 账号，导入后的原始分析全文随代码提供。

本轮复用已完成且音频、系统 Prompt、用户消息及有效模型配置的请求哈希完全一致的返回，补跑缺少的组合；原始响应与调用记录保留。只有两模型、两版 Prompt 的 120 份返回全部正常完成，才会把完整结果导入盲评页面。实际完成数、复用数及格式诊断以 `docs/evaluation-run.json` 的发布记录为准。
...
1. 试听当前音频，阅读匿名 A、B、C、D 四份分析。可切换整体描述、环境与声源、时间变化和完整原文。
...
第四轮比较以下四个方案，音频清单保持原样：

| Prompt | 模型 1 | 模型 2 |
| --- | --- | --- |
| 结构化 | Doubao Seed 2.0 Lite(260428) | Qwen3.5-Omni-Plus |
| Wynn结构化（Schema 2.2） | Doubao Seed 2.0 Lite(260428) | Qwen3.5-Omni-Plus |

`src/data/evaluation.json` 已保存本轮 120 份分析，`public/audio/` 保存原 30 条音频。本轮使用新的数据集 ID；旧轮次评测记录仍保留在各自浏览器存储中，不混入新一轮导出。悬浮播放器、匿名排序、保存及 CSV 导出沿用现有界面。
...
先核实聚合 API 的模型及音频能力，再运行批量请求。`publish_results.py` 只接受完整、成功的 120 份结果；请求失败、缺失或截断时保留已有页面，补跑完成后才能导入。

```sh
python3 evaluation/setup_credentials.py
python3 evaluation/run_eval.py --check-api
python3 evaluation/run_eval.py --output-dir evaluation/results/round4_two_prompts_doubao_plus --workers 6 --per-model-workers 6 --rpm 20 --timeout 300
python3 evaluation/publish_results.py --run-dir evaluation/results/round4_two_prompts_doubao_plus
```
...
- `evaluation/prompts.py` 内置结构化与 Wynn结构化（Schema 2.2）两版完整 Prompt，`prompt_sources.json` 记录原文件哈希。Wynn 文件只移除最外层 Markdown 围栏，参考文本占位符填“未提供”。每个 Prompt 组内，两模型收到相同的系统文本、音频字节及实测音频元数据，不发送来源字幕或参考答案。
- 默认使用 `evaluation/cases.csv` 的固定 30 条，结果目录为 `evaluation/results/round4_two_prompts_doubao_plus`。先试一条可加 `--limit 1 --output-dir evaluation/results/round4_smoke`；预览使用 `--dry-run`，真实任务与预览使用不同目录。
- 第四轮 `evaluation/results/round4_two_prompts_doubao_plus/results.csv` 每行一条音频，共 5 列：音频链接、结构化 Doubao、结构化 Plus、Wynn Doubao、Wynn Plus。同一 Prompt 的两个模型相邻。尚未成功返回的模型单元格为空，原因单独写入状态文件；CSV 保留已有模型完整返回。导入后匿名 A–D 顺序按评审者稳定随机，导出保留真实模型与 Prompt 对应关系。
- 相同命令默认断点续跑；缓存同时校验音频、Prompt、模型配置。只有临时 API 故障会自动重试；非 JSON 原文不修复、不因为格式差而重新抽样。`--rerun` 会重新调用成功项。
- `publish_results.py` 默认读取本轮独立目录，校验完整 30×2×2 矩阵、结构化与 Wynn Prompt 顺序、模型顺序及音频哈希后更新本地页面；失败或截断时停止更新。正常结束但 JSON 格式异常的返回会原样进入评测，异常单独写入报告。
- 历史 `evaluation/results/full` 与 `evaluation/results/round3_wynn_doubao_plus` 目录保留，不用于本轮输出。旧模型全文还可从 Git 历史提交读取。复用旧返回时，必须逐项匹配音频、Prompt 和有效调用配置的请求哈希，并保留原文及调用记录。`reuse_manifest.json` 记录复用来源与响应文件 SHA-256；发布时再次校验当前文件，重跑后不再匹配的返回不计为复用。
