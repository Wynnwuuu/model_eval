...
The shared implementation is `src/insightExports.ts`; focused regression coverage is `scripts/test-insight-exports.ts`. ExcelJS 4.4.0 is a browser-only dynamic chunk, and only its uuid child is overridden to 11.1.1 to remove the new dependency audit finding. No database migration or server API change was added.
